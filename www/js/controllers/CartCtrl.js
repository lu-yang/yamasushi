  ...
  .controller('ProductListCtrl', function($scope, $http, $stateParams, $ionicModal, $localStorage, $helpers) {
          ...
          if ($scope.product.attributionGroups[i].attributionGroupStatus == 1) {
            ...
            if ($scope.product.attributionGroups[i].attributionGroupType == 1) {
              attrValue[i] = {
                ...
                name: $scope.product.attributionGroups[i].attributions[0].attributionName,
                price: $scope.product.attributionGroups[i].attributions[0].attributionPrice
              }
            }

            if ($scope.product.attributionGroups[i].attributionGroupType == 2) {

              checkItem[i] = $scope.product.attributionGroups[i].attributions;
    ...
    $scope.updateValue = function(index, item) {
      attrValue[index] = {
        ...
        price: item.attributionPrice
      };
      $scope.attrValue = attrValue;
      console.log($scope.attrValue);
      console.log('Group : ' + index + 'newValue : ' + item.attributionName);
    }
        ...
        if ($scope.product.attributions) {
          $scope.cart.push({
            ...
            "orderAttributions": [{
              "count": $scope.currentCount,
              "attribution": {
                "id": $scope.product.attributions[$scope.attrValue.key].id
              }
            }],
            "productInfo": {
